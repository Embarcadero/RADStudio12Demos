...
uses Winapi.ActiveX, System.SysUtils, Winapi.Windows;
...
begin
  Result := 0;
  Winapi.Windows.RegOpenKey(Key, PChar(SubKey), Result);
end;
...
begin
  OutKey := 0;
  Result := Winapi.Windows.RegOpenKey(Key, PChar(SubKey), OutKey) = ERROR_SUCCESS;
end;
...
begin
  Result := -1;
  if Winapi.Windows.RegQueryInfoKey(Key, nil, nil, nil, @NumSubKeys, nil, nil, nil,
...
begin
  BufSize := SizeOf(Buffer);
  if Winapi.Windows.RegQueryValue(Key, nil, Buffer, BufSize) = 0 then
  begin
    Result := Buffer;
  end
  else
...
begin
  BufSize := SizeOf(Buffer);
  if Winapi.Windows.RegQueryValueEx(Key, PChar(ValName), nil,
